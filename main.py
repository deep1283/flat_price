        ...
        <head>
            ...
            <style>
                * {{ margin:0; padding:0; box-sizing:border-box; }}
                ...
                .nav-button {{ flex:1; padding:10px 15px; background: linear-gradient(135deg, #28a745 0%, #20c997 100%); color:white; text-decoration:none; border-radius:6px; font-size:13px; font-weight:600; text-align:center; transition: transform 0.2s, box-shadow 0.2s; min-width:120px; }}
                .nav-button:hover {{ transform: translateY(-1px); box-shadow: 0 3px 15px rgba(40, 167, 69, 0.4); }}
                label {{ display:block; margin-bottom:8px; color:#555; font-weight:500; }}
                input[type=\"number\"] {{ width:100%; padding:12px 15px; border:2px solid #e0e0e0; border-radius:8px; font-size:16px; transition:border-color 0.3s; margin-bottom:20px; }}
                ...
                .info {{ margin-top:20px; padding:15px; background:#f5f5f5; border-radius:8px; font-size:13px; color:#666; }}
        ...
        <img src=\"/static/hero.jpeg\" alt=\"hero\" style=\"width:100%; border-radius:8px; box-shadow:0 4px 20px rgba(0,0,0,0.1); margin-bottom:20px;\" />
            ...
            <div class=\"container\">
                ...
                <form method=\"post\">
                    ...
                    <input type=\"number\" id=\"area\" name=\"area\" placeholder=\"e.g., 1000\" min=\"{int(min_area)}\" max=\"{int(max_area)}\" required />
        ...
        <head>
            ...
            <style>
                * {{ margin:0; padding:0; box-sizing:border-box; }}
                ...
                .chart-container img {{ max-width:100%; height:auto; border-radius:8px; box-shadow:0 4px 20px rgba(0,0,0,0.1); }}
                .back-button {{ display:inline-block; padding:12px 25px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color:white; text-decoration:none; border-radius:8px; font-weight:600; transition: transform 0.2s; margin-bottom:20px; }}
...
async def predict(area: int = Form(...)):
    try:
        ...
        if area < min_area * 0.5 or area > max_area * 1.5:
            return error_page(f"Area {area:,} sqft is outside the reliable prediction range", f"Please enter a value between {int(min_area):,} and {int(max_area):,} sqft")
        predicted_price = model.predict([[area]])[0]
        warning = ""
        if area < min_area or area > max_area:
            warning = f"""
                <div class=\"warning\">⚠️ This area is outside our training data range. Prediction may be less accurate.</div>
            """
        return f"""
        <!DOCTYPE html>
        <html>
            <head>
                ...
                <style>
                    * {{ margin:0; padding:0; box-sizing:border-box; }}
                    body {{ font-family:'Segoe UI', Tahoma, Geneva, Verdana, sans-serif; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); min-height: 100vh; display:flex; align-items:center; justify-content:center; padding:20px; }}
                    .container {{ background:white; padding:40px; border-radius:15px; box-shadow:0 10px 40px rgba(0,0,0,0.2); max-width:450px; width:100%; text-align:center; }}
                    h2 {{ color:#333; margin-bottom:10px; font-size:24px; }}
                    .price {{ font-size:48px; font-weight:bold; color:#667eea; margin:20px 0; animation: fadeIn 0.5s; }}
                    .details {{ background:#f5f5f5; padding:20px; border-radius:8px; margin:20px 0; }}
                    .detail-item {{ display:flex; justify-content:space-between; margin:10px 0; color:#555; }}
                    .label {{ font-weight:500; }}
                    .value {{ font-weight:600; color:#333; }}
                    .button {{ display:inline-block; padding:14px 30px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color:white; text-decoration:none; border-radius:8px; font-weight:600; transition: transform 0.2s, box-shadow 0.2s; margin:5px; }}
                    .button.secondary {{ background: linear-gradient(135deg, #28a745 0%, #20c997 100%); }}
                    .warning {{ background:#fff3cd; border:1px solid #ffc107; padding:15px; border-radius:8px; margin:20px 0; color:#856404; font-size:14px; }}
            ...
            <body>
                <div class=\"container\">
                    ...
                    {warning}
                    <div class=\"details\">
                        <div class=\"detail-item\"><span class=\"label\">Area:</span><span class=\"value\">{area:,} sqft</span></div>
                        <div class=\"detail-item\"><span class=\"label\">Price per sqft:</span><span class=\"value\">₹{predicted_price/area:,.0f}</span></div>
                        <div class=\"detail-item\"><span class=\"label\">Model Accuracy:</span><span class=\"value\">{r2_score:.1%}</span></div>
                    </div>
        ...
        <head>
            ...
            <style>
                * {{ margin:0; padding:0; box-sizing:border-box; }}
                ...
                .button {{ display:inline-block; padding:14px 30px; background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); color:white; text-decoration:none; border-radius:8px; font-weight:600; transition: transform 0.2s; }}
